...
class TRTTrimControls(TRTAbstractWidget):
	...
	def __init__(self, ui_manager:object, head_trim:str|None=None, tail_trim:str|None=None):
		...
		self._grp_trim_head = self._ui.HGroup([
			self._lbl_trim_head,
			self._txt_trim_head,
		...
		self._grp_trim_tail = self._ui.HGroup([
			self._lbl_trim_tail,
			self._txt_trim_tail,
			self._ui.HGap({"Weight":0,"MaximumSize":[20,20]}),
		...
		self._ffoa_input_controller = timecodeinput.TRTTimecodeInputController(self._txt_trim_head, allow_negative=False, as_duration=True)
		self._lfoa_input_controller = timecodeinput.TRTTimecodeInputController(self._txt_trim_tail, allow_negative=False, as_duration=True)
	...
	def ffoa_input_controller(self) -> timecodeinput.TRTTimecodeInputController:

		return self._ffoa_input_controller

	def lfoa_input_controller(self) -> timecodeinput.TRTTimecodeInputController:

		return self._lfoa_input_controller
	...
	def set_ffoa_trim_text(self, formatted_duration:str):
		...
		self._ffoa_input_controller.set_from_string(formatted_duration)
	...
	def set_lfoa_trim_text(self, formatted_duration:str):
		...
		self._lfoa_input_controller.set_from_string(formatted_duration)
